                ...
                <div className="fixed inset-0 pt-16 bg-black/60 backdrop-blur-sm z-[100] flex items-end justify-center" onClick={() => setShowModal(null)}>
                    <div className="w-full max-w-md bg-background rounded-t-[2.5rem] p-6 pb-12 space-y-4 max-h-[calc(100vh-4rem)] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                        ...
                        {showModal === "notifications" && (
                            <div className="space-y-3">
                                <ToggleRow label="Novos alunos" defaultOn />
                                <ToggleRow label="Updates de sistema" />
                                <ToggleRow label="Relatórios semanais" defaultOn />
                            </div>
                        )}
                        {showModal === "privacy" && (
                            <div className="space-y-3">
                                <ToggleRow label="Perfil visível" defaultOn />
                                <ToggleRow label="Compartilhar agenda" />
                                <p className="text-xs text-muted-foreground text-center pt-2">Seus dados de professor estão seguros conosco.</p>
                            </div>
                        )}
                    </div>
                </div>
